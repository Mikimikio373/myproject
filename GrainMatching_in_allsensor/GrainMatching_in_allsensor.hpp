...
//注意 :nput1 - inpu2を計算
void GrainMatching_in_stage(const vector<grain>& input1, const vector<grain>& input2, vector<grain_match>& out)
{
    /*type = 0 flag=3をpass, type=1 カット, type=2 全部入れる*/
    ...
    for (int i = 0; i < input1.size(); i++)
    {
        ...
        {
            grain_match dist;
            dist.dx = input1.at(i).x - input2.at(j).x;
            dist.dy = input1.at(i).y - input2.at(j).y;
            if (fabs(dist.dx) > 0.05 || fabs(dist.dy) > 0.05)
            {
                continue;
            }
            dist.x1 = input1.at(i).x;
            dist.y1 = input1.at(i).y;
            dist.x2 = input2.at(j).x;
            dist.y2 = input2.at(j).y;
            out.push_back(dist);
        }
    }
}
...
void grain2csv(string filepath, const vector<grain_match>& input, vector<string> label)
{
    ...
    for (int i = 0; i < sz; i++)
    {
        fprintf(fp, "%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%d\r\n", input[i].dx, input[i].dy, input[i].x1, input[i].y1, input[i].x2, input[i].y2, input[i].flg);
    }
    fclose(fp);
