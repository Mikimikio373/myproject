...
int getopt(vector<string>& input, int& minus, int& target_num_max, int& target_num_min, int& json_flg, double &sx, double &sy) {
    ...
    for (int i = 0; i < input.size(); i++)
    {
        ...
        else if (input[i] == "-nojson")
        {   
            try
            {
                json_flg = 1;
                sx = std::stod(input.at(i + 1));
                sy = std::stod(input.at(i + 2));
                flg = 1;
            }
            catch (const std::exception& tmp)
            {
